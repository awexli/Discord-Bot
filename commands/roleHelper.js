function roleCheck(message, args, commandName) {
    ...
    if (commandName === 'role-' && _hasRole == true) {
        ...
        return message.reply(`\`${guildMember.displayName}\` has been REMOVED from the role, \`${memberRole.name}\``);
    }

    if (commandName === 'role+' && _hasRole !== true) {
        guildMember.addRole(memberRole);
        return message.reply(`\`${guildMember.displayName}\` has been ADDED to the role, \`${memberRole.name}\``);
    }
